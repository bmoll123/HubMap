...
class MaskDINO(nn.Module):
    ...
    def from_config(cls, cfg):
        ...
        if deep_supervision:
            ...
            ti_layers = cfg.MODEL.MaskDINO.TI_LAYERS
            oldm_aux_weight_dict = {}
            for i in range(ti_layers):
                oldm_aux_weight_dict.update({"oldm_" + k + f"_{i}": v for k, v in weight_dict.items()})
            weight_dict.update(oldm_aux_weight_dict)
    ...
    def instance_inference(self, mask_cls, mask_pred, mask_box_result):
        ...
        result.scores = scores_per_image * mask_scores_per_image  # cls得分*mask得分
