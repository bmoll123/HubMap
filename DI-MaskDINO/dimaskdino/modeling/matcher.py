...
from dimaskdino.utils.box_ops import generalized_box_iou,box_cxcywh_to_xyxy
    ...
    def memory_efficient_forward(self, outputs, targets, cost=["cls", "box", "mask"]):
        ...
        for b in range(bs):
            ...
            if 'box' in cost:
                ...
                cost_bbox = torch.tensor(0).to(out_prob)
                cost_giou = torch.tensor(0).to(out_prob)
            ...
            if 'mask' in cost:
                ...
                cost_mask = torch.tensor(0).to(out_prob)
                cost_dice = torch.tensor(0).to(out_prob)
