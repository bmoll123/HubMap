...
from dimaskdino.utils import box_ops
...
class SetCriterion(nn.Module):
    ...
    def forward(self, outputs, targets, mask_dict=None):
        ...
        if "oldm_outputs" in outputs:
            for i, oldm_outputs in enumerate(outputs["oldm_outputs"]):
                indices = self.matcher(oldm_outputs, targets, cost=["cls", "box"])
                for loss in ["labels", "boxes"]:
                    l_dict = self.get_loss(loss, oldm_outputs, targets, indices, num_masks)
                    l_dict = {"oldm_" + k + f"_{i}": v for k, v in l_dict.items()}
                    losses.update(l_dict)
