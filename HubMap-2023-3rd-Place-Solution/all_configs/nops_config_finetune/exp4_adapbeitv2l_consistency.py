...
# ─── 新增：DualScaleHTC 參數 ──────────────────────────────────────────────────
# scale_factor=0.5：Branch B 的輸入縮小至 0.5x（等效 "coarser patch size"）
# consistency_loss_weight=0.1：consistency loss 權重，可視 loss log 調整
# num_fpn_levels=4：對 P2, P3, P4, P5 計算 MSE consistency loss（P6 不算）
# ─────────────────────────────────────────────────────────────────────────────

model = dict(
    type='DualScaleHTC',          # ← 由 HybridTaskCascade 改為 DualScaleHTC
    scale_factor=0.5,
...
# 自動尋找最新的 consistency stage1 權重
stage1_ckpts = sorted(glob.glob('results/consistency/stage1/best_segm_mAP_epoch_*.pth'))
if stage1_ckpts:
    load_from = stage1_ckpts[-1]
else:
    load_from = 'results/consistency/stage1/best_segm_mAP_epoch_8.pth'

work_dir = './results/consistency/stage2'
workflow = [('train', 1)]
auto_resume = False
resume_from = None
launcher = 'none'
